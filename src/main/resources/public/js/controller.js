...
function RbsController($scope, template, model, date, route){
  ...
  $scope.deleteTypesInStructures = function() {
    ...
    $scope.updatePeriodicSummary = function() {
        ...
        $scope.editedBooking.periodicSummary = $scope.summaryBuilsDaysSpecialSlot();
        $scope.editedBooking.periodicShortSummary = lang.translate('rbs.period.days.some');

        if($scope.showDaySelection) {
            $scope.editedBooking.periodicSummary += ', ' + lang.translate('rbs.period.days.then');
            // Selected days
            var selected = 0;
    ...
    $scope.summaryBuilsDaysSpecialSlot = function() {
        var summary = undefined;
        var startDOW = moment($scope.booking.startDate).day();
        var endDOW = moment($scope.booking.endDate).day();

        if (startDOW !== endDOW) {
            summary = lang.translate('rbs.period.days.range.start') + ' ' + lang.translate('rbs.period.days.' + startDOW);
            summary+= lang.translate('rbs.period.days.range.to') + ' ' + lang.translate('rbs.period.days.' + endDOW);
        } else {
            summary = lang.translate('rbs.period.days.one.start') + ' ' + lang.translate('rbs.period.days.' + startDOW);
        }
    ...
    $scope.summaryWriteRange = function(summary, first, last) {
        if (first.number == last.number) {
            ...
            if (summary === undefined) {
                // Start the summary
                return lang.translate('rbs.period.days.one.continue') + ' ' + lang.translate('rbs.period.days.' + first.number);
            }
    ...
    $scope.removeBookingSelection = function() {
        ...
        if (totalSelectionAsynchroneCall === 0) {
            //confirm message
            if ($scope.currentBookingSelected.isSlot() && $scope.currentBookingSelected.booking._slots.length !== 1) {
                $scope.isViewBooking = false;
                $scope.showDeletePeriodicBookingMessage();
            }
            else if ($scope.currentBookingSelected.isSlot() && $scope.currentBookingSelected.booking._slots.length === 1) {
                $scope.isViewBooking = false;
    ...
    $scope.checkDateFunction = function() {
        if( moment($scope.booking.endDate).diff(moment($scope.booking.startDate), 'days') >= 1 ) {
            $scope.showDaySelection = false;
        }
        else {
            $scope.showDaySelection = true;
        }
        if( moment($scope.booking.endDate).diff(moment($scope.booking.periodicEndDate), 'days') > 0 ) {
